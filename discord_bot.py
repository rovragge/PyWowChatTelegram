...
class DiscordBot(Bot):
    ...
    async def handle_CALENDAR_EVENT_CREATION(self, data):
        ...
        for guild in self.guilds:
            ...
            embed = discord.Embed(title=data.title,
                                  ...
                                  description=data.time.strftime('%d/%m/%Y %H:%M') + ' МСК')
            ...
            embed.add_field(name='Сообщение', value=data.text, inline=False)
            invites = ''
            for invite in data.invites:
                player = glob.players.get(invite.guid)
                invites += f'{self.emoji_map[player.char_class]} {player.name} {invite.get_status_emoji()}\n\n'
            embed.add_field(name='Подписаны', value=invites[:-2] or 'Empty', inline=True)
