...
class GameConnector(WoWConnector):
    ...
    async def run(self):
        glob.logger.info(f'Connecting to game server: {glob.realm.address.name}')
        try:
            self.reader, self.writer = await asyncio.open_connection(glob.realm.address.host, glob.realm.address.port)
