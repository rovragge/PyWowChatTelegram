...
class RealmConnector:
    ...
    async def send(self, packet):
        ...
        print(f'SEND REALM PACKET: {packet.id:04X} - {bytes_to_hex_str(packet.data, True, False)}')
    ...
    def decode(self, data):
        ...
        print(f'RECV REALM PACKET: {packet.id:04X} - {bytes_to_hex_str(packet.data, True, False)}')
        return packet

    def handle_realm_packet(self, packet):
        if not isinstance(packet, Packet):
            print(f'packet is instance of {type(packet)}')
            return
        match packet.id:
            case packets.RealmPackets.CMD_AUTH_LOGON_CHALLENGE:
            ...
            case _:
                print(f'Received packet {packet.id:04X} in unexpected logonState')
        pass
    ...
    def handle_CMD_AUTH_LOGON_CHALLENGE(self, packet):
        ...
        if not packets.AuthResult.is_success(result):
            print(packets.AuthResult.get_message(result))
            raise ValueError
    ...
    def handle_CMD_AUTH_LOGON_PROOF(self, packet):
        ...
        if proof != self.srp_handler.generate_hash_logon_proof():
            print('Logon proof generated by client and server differ. Something is very wrong!')
            return
        else:
            account_flag = byte_buff.get(4)
            print(f'Successfully logged into realm server. Looking for realm {self.cfg.get_realm()}')
    ...
    def handle_CMD_REALM_LIST(self, packet):
        ...
        if not correct_realm:
            print(f'Realm {config_realm} not found!')
            print('Possible realms:')
            for realm in realms:
                print(realm)
        else:
            print(f'Found realm {correct_realm["name"]}')
